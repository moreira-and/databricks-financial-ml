...
def ouro_tb_mkt_eqt_perf() -> DataFrame:
    ...
    silver = silver.withColumn("trade_date", F.to_date("trade_date")).withColumn(
        "close_price", F.col("close_price").cast("double")
    )
    ...
    silver = silver.withColumn(
        "ret_diario", F.log("close_price") - F.log(F.lag("close_price", 1).over(w_ret))
    )

    # Contagem na janela
    silver = silver.withColumn("n_obs_30d", F.count("ret_diario").over(w_30d))

    # Última data por símbolo
    silver = silver.withColumn("last_update", F.max("trade_date").over(w_last))
    ...
    df = (
        ...
        .withColumn(
            "volatility",
            F.when(F.col("n_obs_30d") >= 2, F.stddev("ret_diario").over(w_30d)),
        )
...
def ouro_tb_mkt_idx_dash() -> DataFrame:
    ...
    silver = (
        silver.withColumn("ref_date", F.to_date("ref_date"))
    ...
    current = (
        silver.withColumn("rn", F.row_number().over(w_last))
        .filter(F.col("rn") == 1)
        .select(
            "idx_type",
    ...
    prev_month_ref = (
        current.withColumn(
            "prev_month",
            F.when(F.col("cur_month") == 1, F.lit(12)).otherwise(
                F.col("cur_month") - 1
            ),
        )
        .withColumn(
            "prev_month_year",
            F.when(F.col("cur_month") == 1, F.col("cur_year") - 1).otherwise(
                F.col("cur_year")
            ),
        )
        .select(
    ...
    w_prev_month_last = Window.partitionBy("idx_type", "year", "month").orderBy(
        F.col("ref_date").desc()
    )

    prev_month_values = (
        silver.withColumn("rn_pm", F.row_number().over(w_prev_month_last))
    ...
    current_with_prev_month = (
        current.join(prev_month_ref, on="idx_type", how="left")
        .join(
            prev_month_values,
    ...
    # ----------------------------------------------------------------------
    prev_year_ref = current.withColumn("prev_year", F.col("cur_year") - 1).select(
        "idx_type", "prev_year"
    )

    w_prev_year_last = Window.partitionBy("idx_type", "year").orderBy(
        F.col("ref_date").desc()
    )

    prev_year_values = (
        silver.withColumn("rn_py", F.row_number().over(w_prev_year_last))
    ...
    current_with_prev = (
        current_with_prev_month.join(prev_year_ref, on="idx_type", how="left")
        .join(
            prev_year_values,
...
def ouro_tb_mkt_idx_fut_perf() -> DataFrame:
    ...
    silver = (
        silver.withColumn("trade_date", F.to_date("trade_date"))
    ...
    silver = silver.withColumn(
        "n_obs_30d", F.count("ret_diario").over(w_30d)
    ).withColumn("n_obs_ytd", F.count("ret_diario").over(w_ytd))

    # Última data por símbolo
    silver = silver.withColumn(
        "last_update", F.max("trade_date").over(Window.partitionBy("symbol"))
    )
    ...
    df = (
        ...
        .withColumn(
            "price_52w_high", F.last("price_52w_high", ignorenulls=True).over(w_all)
        )
        .withColumn(
            "price_52w_low", F.last("price_52w_low", ignorenulls=True).over(w_all)
        )
        .withColumn(
            "pct_from_52w_high",
            F.when(
                (F.col("price_52w_high").isNotNull()) & (F.col("price_52w_high") != 0),
                (F.col("last_close") - F.col("price_52w_high"))
                / F.col("price_52w_high"),
            ),
        )
        .withColumn(
            "pct_from_52w_low",
